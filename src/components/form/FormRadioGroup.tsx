            ...
            <RadioGroup onValueChange={field.onChange} value={field.value} className={cn("grid grid-cols-1 gap-3", size === "sm" ? "grid-cols-2" : "grid-cols-2")}>
               ...
               {options.map(({ value, label }) => (
                  <div key={value} className="flex-1">
                     ...
                     <Label htmlFor={value} className={cn("flex items-center border-1 border-gray-200 rounded-md cursor-pointer transition-all peer-data-[state=checked]:bg-teal-100 peer-data-[state=checked]:border-teal-500", size === 'sm' ? "p-3.5" : "p-4")}>
