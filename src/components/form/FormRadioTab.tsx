               ...
               {options.map(({ value, label, icon: Icon }) => {
                  ...
                  return(
                     <div key={value}>
                        ...
                        <Label
                           htmlFor={`${name}-${value}`}
                           className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[0.85rem] font-medium cursor-pointer transition-all duration-200", isSelected ? "border-teal-500 bg-teal-100 text-teal-600" : "border-gray-200 text-gray-500 hover:border-teal-300 hover:text-teal-600")}>
                              {isSelected ? <Check className="size-4" /> : <Icon className="size-3.5" />}
                              {label}
                           </Label>
                     </div>
                  )
