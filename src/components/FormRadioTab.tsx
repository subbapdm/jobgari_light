            ...
            <RadioGroup onValueChange={field.onChange} value={field.value} className="flex items-center justify-between bg-slate-100 p-1.5 rounded-md">
               ...
               {options.map(({ value, label }) => (
                  <div key={value} className="flex-1">
                     ...
                     <Label htmlFor={value} className={cn("flex items-center justify-center rounded-sm cursor-pointer transition-all peer-data-[state=checked]:bg-white peer-data-[state=checked]:border-teal-500 p-3")}>
